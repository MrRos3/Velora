...
-- Uses the latest proven Velora UI snapshot, but forces the old fast/local public song path.
...
-- Keep the new UI, but point its registry/song loading back to the public local library.
source = replaceOnce(
    source,
...
-- Main/Songs.lua has normal File= entries, so the runtime naturally takes the fast local path.
