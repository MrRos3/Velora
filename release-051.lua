...
local function parseSheet(sheet, bpm, stepsPerBeat)
    ...
    for token in tostring(sheet or ""):gmatch("%S+") do
        if token == "|" then
            -- A bar line is also a breath. Dense arrangements no longer run
            -- every phrase directly into the next one.
            cursor += stepDuration
...
local function loadRegistry()
    local registry = safeLoadTable(RAW_BASE .. "Songs.lua?velora=0.5.1")
...
-- Corner glow blobs were removed: clipping round shapes against the window
-- produced sharp wedges. Smooth gradients now carry the color instead.
...
local accentGradients={
    logo:FindFirstChildOfClass("UIGradient"),art:FindFirstChildOfClass("UIGradient"),
    play:FindFirstChildOfClass("UIGradient"),fill:FindFirstChildOfClass("UIGradient"),
    applyColorButton:FindFirstChildOfClass("UIGradient"),window:FindFirstChildOfClass("UIGradient"),
    header:FindFirstChildOfClass("UIGradient"),
}
local themedStrokes={
    window:FindFirstChildOfClass("UIStroke"),header:FindFirstChildOfClass("UIStroke"),
    nav:FindFirstChildOfClass("UIStroke"),browser:FindFirstChildOfClass("UIStroke"),
    playerCard:FindFirstChildOfClass("UIStroke"),palette:FindFirstChildOfClass("UIStroke"),
}
local function applyAccent(color)
    ...
    feedback.BackgroundColor3=deep:Lerp(P.Card,.45)
...
animate(window,{Size=UDim2.fromOffset(760,440)},.3)
