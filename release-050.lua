...
local function loadRegistry()
    local registry = safeLoadTable(RAW_BASE .. "Songs.lua?velora=0.5.0")
...
local shadow=radius(make("Frame",{AnchorPoint=Vector2.new(.5,.5),Position=UDim2.new(.5,0,.5,4),Size=UDim2.fromOffset(768,448),BackgroundColor3=Color3.new(0,0,0),BackgroundTransparency=.78,BorderSizePixel=0},gui),28)
...
local logoText=label(logo,"🥀",UDim2.fromScale(0,0),UDim2.fromScale(1,1),Enum.Font.Gotham,21,P.Text);logoText.TextXAlignment=Enum.TextXAlignment.Center
...
local settingsButton=radius(edge(make("TextButton",{Position=UDim2.new(1,-98,0,14),Size=UDim2.fromOffset(36,36),BackgroundColor3=Color3.fromRGB(49,42,65),BorderSizePixel=0,AutoButtonColor=false,Font=Enum.Font.GothamBold,Text="⚙",TextSize=15,TextColor3=P.Sub},header),Color3.fromRGB(105,94,143),.55),12)
...
local bpmDown=radius(make("TextButton",{Position=UDim2.fromOffset(4,4),Size=UDim2.fromOffset(30,30),BackgroundColor3=P.Lift,BorderSizePixel=0,AutoButtonColor=false,Font=Enum.Font.GothamBold,Text="‹",TextSize=19,TextColor3=P.Sub},bpmPill),10)
local bpm=make("TextBox",{Position=UDim2.fromOffset(38,0),Size=UDim2.fromOffset(52,38),BackgroundTransparency=1,ClearTextOnFocus=false,Font=Enum.Font.GothamBlack,Text="120",TextSize=10,TextColor3=P.Text},bpmPill)
local bpmUp=radius(make("TextButton",{Position=UDim2.fromOffset(94,4),Size=UDim2.fromOffset(30,30),BackgroundColor3=P.Lift,BorderSizePixel=0,AutoButtonColor=false,Font=Enum.Font.GothamBold,Text="›",TextSize=19,TextColor3=P.Sub},bpmPill),10)
...
label(palette,"PALETTE STUDIO",UDim2.fromOffset(18,16),UDim2.fromOffset(190,22),Enum.Font.GothamBlack,13,P.Text).ZIndex=51
label(palette,"Shape Velora around your favorite color.",UDim2.fromOffset(18,39),UDim2.fromOffset(235,16),Enum.Font.Gotham,8,P.Sub).ZIndex=51
local paletteClose=radius(make("TextButton",{Position=UDim2.new(1,-45,0,12),Size=UDim2.fromOffset(31,31),BackgroundColor3=P.Card,BorderSizePixel=0,Text="×",Font=Enum.Font.GothamBold,TextSize=16,TextColor3=P.Sub,ZIndex=52},palette),10)
...
for index,name in ipairs({"R","G","B"}) do
    label(palette,name,UDim2.fromOffset(18+(index-1)*82,129),UDim2.fromOffset(20,16),Enum.Font.GothamBold,8,P.Muted).ZIndex=51
    rgbInputs[index]=radius(make("TextBox",{Position=UDim2.fromOffset(18+(index-1)*82,148),Size=UDim2.fromOffset(72,36),BackgroundColor3=P.Card,BorderSizePixel=0,ClearTextOnFocus=false,Text=({"143","108","255"})[index],Font=Enum.Font.GothamBold,TextSize=10,TextColor3=P.Text,ZIndex=51},palette),11)
end
label(palette,"HEX",UDim2.fromOffset(18,202),UDim2.fromOffset(35,16),Enum.Font.GothamBold,8,P.Muted).ZIndex=51
...
local applyColorButton=radius(gradient(make("TextButton",{Position=UDim2.fromOffset(18,276),Size=UDim2.fromOffset(244,31),BackgroundColor3=P.Violet,BorderSizePixel=0,Text="APPLY COLOR",Font=Enum.Font.GothamBlack,TextSize=9,TextColor3=P.Text,ZIndex=51},palette),P.Violet,P.Pink,0),11)
...
local accentGradients={logo:FindFirstChildOfClass("UIGradient"),art:FindFirstChildOfClass("UIGradient"),play:FindFirstChildOfClass("UIGradient"),fill:FindFirstChildOfClass("UIGradient"),applyColorButton:FindFirstChildOfClass("UIGradient")}
local function applyAccent(color)
    ...
    for _,g in ipairs(accentGradients) do if g then g.Color=ColorSequence.new(color,P.Pink) end end
    if navButtons[activeFilter] then navButtons[activeFilter].BackgroundColor3=color:Lerp(P.Ink,.48) end
end
...
UserInputService.InputChanged:Connect(function(input)
    if dragging and (input.UserInputType==Enum.UserInputType.MouseMovement or input.UserInputType==Enum.UserInputType.Touch) then local delta=input.Position-dragStart;window.Position=UDim2.new(startPos.X.Scale,startPos.X.Offset+delta.X,startPos.Y.Scale,startPos.Y.Offset+delta.Y);shadow.Position=UDim2.new(window.Position.X.Scale,window.Position.X.Offset,window.Position.Y.Scale,window.Position.Y.Offset+4) end
end)
...
render()
window.Size=UDim2.fromOffset(710,400);shadow.Size=UDim2.fromOffset(718,408)
animate(window,{Size=UDim2.fromOffset(760,440)},.3);animate(shadow,{Size=UDim2.fromOffset(768,448)},.3)
