...
local function loadRegistry()
    local registry = safeLoadTable(RAW_BASE .. "Songs.lua?velora=0.6.0")
...
refreshList=function()
    ...
    for index,entry in ipairs(state.Registry) do
        if entryMatches(entry) then
            ...
            local cardMusicIcon=icon(tile,"music-2",17,P.Text,"");cardMusicIcon.AnchorPoint=Vector2.new(.5,.5);cardMusicIcon.Position=UDim2.fromScale(.5,.5)
            ...
            local cardArrowIcon=icon(card,"chevron-right",17,Color3.fromRGB(188,172,255),"");cardArrowIcon.AnchorPoint=Vector2.new(.5,.5);cardArrowIcon.Position=UDim2.new(1,-24,.5,0)
